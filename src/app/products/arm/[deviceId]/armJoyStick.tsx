...
export default function ArmJoyStick({device_id , isLoading ,client,isConnected,topic}:Props) {
        ...
        <div  className={`grid grid-rows-2 w-2/4 ${isLoading? '':''}  `}>
            <div className='grid grid-cols-[50%_50%] gap-2 mb-2'>
                ...
                <div className={`duration-1000 grid place-items-center shadow-md shadow-gray-900  rounded-full ${isLoading? 'bg-blue-800':'bg-gray-500'} `}>
                    {hold == false ? (
                        <button className={`text-4xl duration-75 bg-gray-800 active:bg-violet-700  shadow-md shadow-gray-900 text-white  rounded-full py-8 px-2 ${isLoading? 'animate-fadeIn':'opacity-0'} `} onClick={Hold}>
                        HOLD
                    </button>   
                    ):(
                    <button className={`text-4xl duration-75 bg-green-600 active:bg-violet-700  shadow-md shadow-gray-900 text-white  rounded-full py-8 px-2 ${isLoading? 'animate-fadeIn':'opacity-0'} `} onClick={Hold}>
                        HOLD
                    </button>  
                    )}
